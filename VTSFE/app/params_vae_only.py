...
data_path = "/media/mchavero/F8CA0E9ECA0E5970/xsens_experiments_data/xml"
data_types = ["position"]
plot_3D = True

sub_sequences_size = 1
use_whole_sequence_for_forcing_term = False
continuity_penality = None
use_z_derivative = False
transform_with_all_dvbf = False
...
DMP_PARAMS = {
    ...
    "activation_function": tf.nn.softmax,   # softmax. activation function used in article, but since it rescales input values to range [0, 1], all observations look the same => f is approximately the same for all movement types
